...
if uploaded_files:
    ...
    file_base = st.text_input("Choose filename (without extension):", value="spectrum_plot")

    # === CSV Download der Peaks ===
    if show_peaks:
        all_peaks = []
        for name, df in dfs.items():
        ...
        if all_peaks:
            ...
            st.download_button(
                label="Download Negative Peaks as CSV",
                data=csv,
                file_name=f"{file_base}_peaks.csv",
                mime="text/csv"
            )

    # === PNG Download des Plots ===
    buf = io.BytesIO()
    fig.savefig(buf, format="png")
    buf.seek(0)
    st.download_button(
        "Download Spectrum Plot as PNG",
        data=buf,
